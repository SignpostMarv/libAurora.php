<?php
//!	@file Aurora/Addon/WebAPI/Group.php
//!	@brief Group-related WebAPI code
//!	@author SignpostMarv
...
*/
		protected function __construct($uuid, $name, $charter, $insignia, $founder, $membershipFee, $openEnrollment, $showInList, $allowPublish, $maturePublish, $ownerRoleID){
			...
			}else if(preg_match(WebAPI::regex_UUID, $insignia) !== 1){
			...
			}else if(preg_match(WebAPI::regex_UUID, $founder) !== 1){
			...
			}else if(preg_match(WebAPI::regex_UUID, $ownerRoleID) !== 1){
...
*	@return object Aurora::Addon::WebAPI::GroupRecord
*/
...
//!	string group UUID
//!	@see Aurora::Addon::WebAPI::GroupRecord::GroupID()
		protected $GroupID;
//!	@see Aurora::Addon::WebAPI::GroupRecord::$GroupID
...
//!	string group name
//!	@see Aurora::Addon::WebAPI::GroupRecord::GroupName()
		protected $GroupName;
//!	@see Aurora::Addon::WebAPI::GroupRecord::$GroupName
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::Charter()
		protected $Charter;
//!	@see Aurora::Addon::WebAPI::GroupRecord::$Charter
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::GroupPicture()
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::FounderID()
		protected $FounderID;
//!	@see Aurora::Addon::WebAPI::GroupRecord::$FounderID
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::MembershipFee()
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::OpenEnrollment()
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::ShowInList()
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::AllowPublish()
...
//!	@see Aurora::Addon::WebAPI::GroupRecord::MaturePublish()
...
//!	string Role ID
//!	@see Aurora::Addon::WebAPI::GroupRecord::OwnerRoleID()
...
/**
*	@param object $WebAPI instance of Aurora::Addon::WebAPI We need to specify this in case we want to iterate past the original set of results.
...
*	@param array $groups if specified, should be an array of instances of Aurora::Addon::WebAPI::GroupRecord that were pre-fetched with a call to the API end-point.
*/
		protected function __construct(WebAPI $WebAPI, $start=0, $total=0, array $sort=null, array $boolFields=null, array $groups=null){
			parent::__construct($WebAPI, $start, $total, $sort, $boolFields);
			if(isset($groups) === true){
				...
				foreach($groups as $group){
					if($group instanceof GroupRecord){
						...
						throw new InvalidArgumentException('Only instances of Aurora::Addon::WebAPI::GroupRecord should be passed to Aurora::Addon::WebAPI::GetGroupRecords::__construct()');
					}
				}
			}
		}

//!	To avoid slowdowns due to an excessive amount of curl calls, we populate Aurora::Addon::WebAPI::GetGroupRecords::$data in batches of 10
/**
*	@return mixed either NULL or an instance of Aurora::Addon::WebAPI::GroupRecord
*/
		public function current(){
			if($this->valid() === false){
				...
				$start   = $this->key();
				$results = $this->WebAPI->GetGroups($start, 10, $this->sort, $this->boolFields);
...
//!	array Registry array of Aurora::Addon::WebAPI::GetNewsSources for all grids
		protected static $registry = array();

//!	We're hiding this behind a registry method.
		protected function __construct(WebAPI $WebAPI, $start=0, $total=0, array $groups=null){
			if(isset($groups) === true){
				$i = $start;
				foreach($groups as $group){
					if($group instanceof GroupRecord){
						...
						throw new InvalidArgumentException('Values of instantiated groups array must be instances of Aurora::Addon::WebAPI::GroupRecord');
					}
				}
			}
...
//!	registry method
		public static function r(WebAPI $WebAPI, $start=0, $total=null, array $groups=null){
			...
			if(isset(static::$registry[$hash]) === false && isset($total) === false){
				...
				static::$registry[$hash] = new static($WebAPI, $start, $total, $groups);
			}
...
*/
		public static function hasInstance(WebAPI $WebAPI){
			return isset(static::$registry[spl_object_hash($WebAPI)]);
		}

//!	To avoid slowdowns due to an excessive amount of curl calls, we populate Aurora::Addon::WebAPI::GetNewsSources::$data in batches of 10
/**
*	@return mixed either NULL or an instance of Aurora::Addon::WebAPI::GroupRecord
*/
		public function current(){
			if($this->valid() === false){
				...
				$start   = $this->key();
				$results = $this->WebAPI->GetNewsSources($start, 10, true);
				foreach($results as $group){
					$this->data[$start++] = $group;
				}
			}
...
*/
		public function __construct($GroupID, $NoticeID, $timestamp, $FromName, $Subject, $Message, $HasAttachment=false, $ItemID='00000000-0000-0000-0000-000000000000', $AssetType=-1, $ItemName=''){
			...
			}else if(preg_match(WebAPI::regex_UUID, $NoticeID) !== 1){
			...
			}else if(preg_match(WebAPI::regex_UUID, $ItemID) !== 1){
...
*/
		public static function r($GroupID, $NoticeID, $timestamp=null, $FromName=null, $Subject=null, $Message=null, $HasAttachment=false, $ItemID='00000000-0000-0000-0000-000000000000', $AssetType=-1, $ItemName=''){
			if(is_string($GroupID) === false){
				throw new InvalidArgumentException('GroupID must be a string.');
			}else if(preg_match(WebAPI::regex_UUID, $GroupID) === false){
				throw new InvalidArgumentException('GroupID must be a valid UUID.');
			}else if(is_string($NoticeID) === false){
				throw new InvalidArgumentException('NoticeID must be a string.');
			}else if(preg_match(WebAPI::regex_UUID, $NoticeID) === false){
				throw new InvalidArgumentException('NoticeID must be a valid UUID.');
			}
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::GroupID()
		protected $GroupID;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$GroupID
		public function GroupID(){
			return $this->GroupID;
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::NoticeID()
		protected $NoticeID;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$NoticeID
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::Timestamp()
		protected $Timestamp;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$Timestamp
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::FromName()
		protected $FromName;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$FromName
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::Subject()
		protected $Subject;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$Subject
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::Message()
		protected $Message;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$Message
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::HasAttachment()
...
//!	integer asset type
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::AssetType()
		protected $AssetType;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$AssetType
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::ItemID()
		protected $ItemID;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$ItemID
...
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::ItemName()
		protected $ItemName;
//!	@see Aurora::Addon::WebAPI::GroupNoticeData::$ItemName
...
/**
*	@return object an instance of Aurora::Addon::WebAPI::foreknowledgeGetGroupRecords()
*/
...
*	@param array $groupNotices if specified, should be an array of instances of Aurora::Addon::WebAPI::GroupNoticeData that were pre-fetched with a call to the API end-point.
*/
		protected function __construct(WebAPI $WebAPI, $start=0, $total=0, array $groups, array $groupNotices=null){
			...
			parent::__construct($WebAPI, $start, $total, null, null);
			if(isset($groupNotices) === true){
				...
				foreach($groupNotices as $group){
					if($group instanceof GroupNoticeData){
						...
						throw new InvalidArgumentException('Only instances of Aurora::Addon::WebAPI::GroupNoticeData should be passed to Aurora::Addon::WebAPI::GetGroupRecords::__construct()');
					}
				}
			}
		}
...
*	@return object an instance of Aurora::Addon::WebAPI::GetGroupNotices
*/
		public static function r(abstractAPI $WebAPI, $start=0, $total=0, array $groups=null, array $entities=null, array $ignored=null){
			sort($groups);
			static $registry = array();
			...
			if($create === true){
				$registry[$hash1][$hash2] = new static($WebAPI, $start, $total, $groups, $entities);
			}
