<?php
//!	@file Aurora/Addon/WebAPI/abstracts.php
//!	@brief abstract WebAPI classes
//!	@author SignpostMarv
...
//!	abstract iterator for instances of Aurora::Addon::WebAPI::abstractUser
	abstract class abstractUserIterator extends abstractIterator{

//!	public constructor
/**
*	Since Aurora::Addon::WebAPI::abstractUserIterator does not implement methods for appending values, calling the constructor with no arguments is a shorthand means of indicating there are no users available.
*	@param mixed $archives an array of Aurora::Addon::WebAPI::abstractUser instances or NULL
*/
			...
			if(isset($archives) === true){
				foreach($archives as $v){
					if(($v instanceof abstractUser) === false){
						throw new InvalidArgumentException('Only instances of Aurora::Addon::WebAPI::abstractUser should be included in the array passed to Aurora::Addon::WebAPI::abstractUserIterator::__construct()');
					}
				}
				reset($archives);
				$this->data = $archives;
...
//!	object instance of Aurora::Addon::WebAPI
		protected $WebAPI;
...
//!	We're hiding this behind factory or registry methods.
		protected function __construct(WebAPI $WebAPI, $start=0, $total=0){
...
//!	We're not supporting external manipulation of Aurora::Addon::WebAPI::abstractLazyLoadingSeekableIterator::$data
		public function offsetSet($offset, $value){
			throw new BadMethodCallException('Instances of Aurora::Addon::WebAPI::abstractLazyLoadingSeekableIterator cannot be modified from outside of the object scope.');
		}
...
*/
		public function seek($to){
			...
			if(is_integer($to) === false){
				...
				throw new LengthException('Cannot seek past Aurora::Addon::WebAPI::abstractLazyLoadingSeekableIterator::count()');
			}
...
**/
		protected function __construct(WebAPI $WebAPI, $start=0, $total=0, array $regions=null){
			...
			if(isset($regions) === true){
				foreach($regions as $region){
					if($region instanceof GridRegion){
						...
						throw new InvalidArgumentException('Values of instantiated regions array must be instances of Aurora::Addon::WebAPI::GridRegion');
					}
				}
			}
		}
